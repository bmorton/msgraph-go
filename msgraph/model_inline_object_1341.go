/*
Partial Graph API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: v1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package msgraph

import (
	"encoding/json"
)

// InlineObject1341 struct for InlineObject1341
type InlineObject1341 struct {
	Number AnyOfobject `json:"number,omitempty"`
	Significance AnyOfobject `json:"significance,omitempty"`
	Mode AnyOfobject `json:"mode,omitempty"`
}

// NewInlineObject1341 instantiates a new InlineObject1341 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineObject1341() *InlineObject1341 {
	this := InlineObject1341{}
	return &this
}

// NewInlineObject1341WithDefaults instantiates a new InlineObject1341 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineObject1341WithDefaults() *InlineObject1341 {
	this := InlineObject1341{}
	return &this
}

// GetNumber returns the Number field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *InlineObject1341) GetNumber() AnyOfobject {
	if o == nil  {
		var ret AnyOfobject
		return ret
	}
	return o.Number
}

// GetNumberOk returns a tuple with the Number field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *InlineObject1341) GetNumberOk() (*AnyOfobject, bool) {
	if o == nil || o.Number == nil {
		return nil, false
	}
	return &o.Number, true
}

// HasNumber returns a boolean if a field has been set.
func (o *InlineObject1341) HasNumber() bool {
	if o != nil && o.Number != nil {
		return true
	}

	return false
}

// SetNumber gets a reference to the given AnyOfobject and assigns it to the Number field.
func (o *InlineObject1341) SetNumber(v AnyOfobject) {
	o.Number = v
}

// GetSignificance returns the Significance field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *InlineObject1341) GetSignificance() AnyOfobject {
	if o == nil  {
		var ret AnyOfobject
		return ret
	}
	return o.Significance
}

// GetSignificanceOk returns a tuple with the Significance field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *InlineObject1341) GetSignificanceOk() (*AnyOfobject, bool) {
	if o == nil || o.Significance == nil {
		return nil, false
	}
	return &o.Significance, true
}

// HasSignificance returns a boolean if a field has been set.
func (o *InlineObject1341) HasSignificance() bool {
	if o != nil && o.Significance != nil {
		return true
	}

	return false
}

// SetSignificance gets a reference to the given AnyOfobject and assigns it to the Significance field.
func (o *InlineObject1341) SetSignificance(v AnyOfobject) {
	o.Significance = v
}

// GetMode returns the Mode field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *InlineObject1341) GetMode() AnyOfobject {
	if o == nil  {
		var ret AnyOfobject
		return ret
	}
	return o.Mode
}

// GetModeOk returns a tuple with the Mode field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *InlineObject1341) GetModeOk() (*AnyOfobject, bool) {
	if o == nil || o.Mode == nil {
		return nil, false
	}
	return &o.Mode, true
}

// HasMode returns a boolean if a field has been set.
func (o *InlineObject1341) HasMode() bool {
	if o != nil && o.Mode != nil {
		return true
	}

	return false
}

// SetMode gets a reference to the given AnyOfobject and assigns it to the Mode field.
func (o *InlineObject1341) SetMode(v AnyOfobject) {
	o.Mode = v
}

func (o InlineObject1341) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Number != nil {
		toSerialize["number"] = o.Number
	}
	if o.Significance != nil {
		toSerialize["significance"] = o.Significance
	}
	if o.Mode != nil {
		toSerialize["mode"] = o.Mode
	}
	return json.Marshal(toSerialize)
}

type NullableInlineObject1341 struct {
	value *InlineObject1341
	isSet bool
}

func (v NullableInlineObject1341) Get() *InlineObject1341 {
	return v.value
}

func (v *NullableInlineObject1341) Set(val *InlineObject1341) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineObject1341) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineObject1341) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineObject1341(val *InlineObject1341) *NullableInlineObject1341 {
	return &NullableInlineObject1341{value: val, isSet: true}
}

func (v NullableInlineObject1341) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineObject1341) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


