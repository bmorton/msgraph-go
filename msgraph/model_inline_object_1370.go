/*
Partial Graph API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: v1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package msgraph

import (
	"encoding/json"
)

// InlineObject1370 struct for InlineObject1370
type InlineObject1370 struct {
	Inumber AnyOfobject `json:"inumber,omitempty"`
}

// NewInlineObject1370 instantiates a new InlineObject1370 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineObject1370() *InlineObject1370 {
	this := InlineObject1370{}
	return &this
}

// NewInlineObject1370WithDefaults instantiates a new InlineObject1370 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineObject1370WithDefaults() *InlineObject1370 {
	this := InlineObject1370{}
	return &this
}

// GetInumber returns the Inumber field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *InlineObject1370) GetInumber() AnyOfobject {
	if o == nil  {
		var ret AnyOfobject
		return ret
	}
	return o.Inumber
}

// GetInumberOk returns a tuple with the Inumber field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *InlineObject1370) GetInumberOk() (*AnyOfobject, bool) {
	if o == nil || o.Inumber == nil {
		return nil, false
	}
	return &o.Inumber, true
}

// HasInumber returns a boolean if a field has been set.
func (o *InlineObject1370) HasInumber() bool {
	if o != nil && o.Inumber != nil {
		return true
	}

	return false
}

// SetInumber gets a reference to the given AnyOfobject and assigns it to the Inumber field.
func (o *InlineObject1370) SetInumber(v AnyOfobject) {
	o.Inumber = v
}

func (o InlineObject1370) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Inumber != nil {
		toSerialize["inumber"] = o.Inumber
	}
	return json.Marshal(toSerialize)
}

type NullableInlineObject1370 struct {
	value *InlineObject1370
	isSet bool
}

func (v NullableInlineObject1370) Get() *InlineObject1370 {
	return v.value
}

func (v *NullableInlineObject1370) Set(val *InlineObject1370) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineObject1370) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineObject1370) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineObject1370(val *InlineObject1370) *NullableInlineObject1370 {
	return &NullableInlineObject1370{value: val, isSet: true}
}

func (v NullableInlineObject1370) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineObject1370) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


