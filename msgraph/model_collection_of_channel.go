/*
Partial Graph API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: v1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package msgraph

import (
	"encoding/json"
)

// CollectionOfChannel struct for CollectionOfChannel
type CollectionOfChannel struct {
	Value *[]MicrosoftGraphChannel `json:"value,omitempty"`
	OdataNextLink *string `json:"@odata.nextLink,omitempty"`
}

// NewCollectionOfChannel instantiates a new CollectionOfChannel object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCollectionOfChannel() *CollectionOfChannel {
	this := CollectionOfChannel{}
	return &this
}

// NewCollectionOfChannelWithDefaults instantiates a new CollectionOfChannel object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCollectionOfChannelWithDefaults() *CollectionOfChannel {
	this := CollectionOfChannel{}
	return &this
}

// GetValue returns the Value field value if set, zero value otherwise.
func (o *CollectionOfChannel) GetValue() []MicrosoftGraphChannel {
	if o == nil || o.Value == nil {
		var ret []MicrosoftGraphChannel
		return ret
	}
	return *o.Value
}

// GetValueOk returns a tuple with the Value field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CollectionOfChannel) GetValueOk() (*[]MicrosoftGraphChannel, bool) {
	if o == nil || o.Value == nil {
		return nil, false
	}
	return o.Value, true
}

// HasValue returns a boolean if a field has been set.
func (o *CollectionOfChannel) HasValue() bool {
	if o != nil && o.Value != nil {
		return true
	}

	return false
}

// SetValue gets a reference to the given []MicrosoftGraphChannel and assigns it to the Value field.
func (o *CollectionOfChannel) SetValue(v []MicrosoftGraphChannel) {
	o.Value = &v
}

// GetOdataNextLink returns the OdataNextLink field value if set, zero value otherwise.
func (o *CollectionOfChannel) GetOdataNextLink() string {
	if o == nil || o.OdataNextLink == nil {
		var ret string
		return ret
	}
	return *o.OdataNextLink
}

// GetOdataNextLinkOk returns a tuple with the OdataNextLink field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CollectionOfChannel) GetOdataNextLinkOk() (*string, bool) {
	if o == nil || o.OdataNextLink == nil {
		return nil, false
	}
	return o.OdataNextLink, true
}

// HasOdataNextLink returns a boolean if a field has been set.
func (o *CollectionOfChannel) HasOdataNextLink() bool {
	if o != nil && o.OdataNextLink != nil {
		return true
	}

	return false
}

// SetOdataNextLink gets a reference to the given string and assigns it to the OdataNextLink field.
func (o *CollectionOfChannel) SetOdataNextLink(v string) {
	o.OdataNextLink = &v
}

func (o CollectionOfChannel) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Value != nil {
		toSerialize["value"] = o.Value
	}
	if o.OdataNextLink != nil {
		toSerialize["@odata.nextLink"] = o.OdataNextLink
	}
	return json.Marshal(toSerialize)
}

type NullableCollectionOfChannel struct {
	value *CollectionOfChannel
	isSet bool
}

func (v NullableCollectionOfChannel) Get() *CollectionOfChannel {
	return v.value
}

func (v *NullableCollectionOfChannel) Set(val *CollectionOfChannel) {
	v.value = val
	v.isSet = true
}

func (v NullableCollectionOfChannel) IsSet() bool {
	return v.isSet
}

func (v *NullableCollectionOfChannel) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCollectionOfChannel(val *CollectionOfChannel) *NullableCollectionOfChannel {
	return &NullableCollectionOfChannel{value: val, isSet: true}
}

func (v NullableCollectionOfChannel) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCollectionOfChannel) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


