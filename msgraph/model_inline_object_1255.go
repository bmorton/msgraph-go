/*
Partial Graph API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: v1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package msgraph

import (
	"encoding/json"
)

// InlineObject1255 struct for InlineObject1255
type InlineObject1255 struct {
	IndexNum AnyOfobject `json:"indexNum,omitempty"`
	Values AnyOfobject `json:"values,omitempty"`
}

// NewInlineObject1255 instantiates a new InlineObject1255 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineObject1255() *InlineObject1255 {
	this := InlineObject1255{}
	return &this
}

// NewInlineObject1255WithDefaults instantiates a new InlineObject1255 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineObject1255WithDefaults() *InlineObject1255 {
	this := InlineObject1255{}
	return &this
}

// GetIndexNum returns the IndexNum field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *InlineObject1255) GetIndexNum() AnyOfobject {
	if o == nil  {
		var ret AnyOfobject
		return ret
	}
	return o.IndexNum
}

// GetIndexNumOk returns a tuple with the IndexNum field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *InlineObject1255) GetIndexNumOk() (*AnyOfobject, bool) {
	if o == nil || o.IndexNum == nil {
		return nil, false
	}
	return &o.IndexNum, true
}

// HasIndexNum returns a boolean if a field has been set.
func (o *InlineObject1255) HasIndexNum() bool {
	if o != nil && o.IndexNum != nil {
		return true
	}

	return false
}

// SetIndexNum gets a reference to the given AnyOfobject and assigns it to the IndexNum field.
func (o *InlineObject1255) SetIndexNum(v AnyOfobject) {
	o.IndexNum = v
}

// GetValues returns the Values field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *InlineObject1255) GetValues() AnyOfobject {
	if o == nil  {
		var ret AnyOfobject
		return ret
	}
	return o.Values
}

// GetValuesOk returns a tuple with the Values field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *InlineObject1255) GetValuesOk() (*AnyOfobject, bool) {
	if o == nil || o.Values == nil {
		return nil, false
	}
	return &o.Values, true
}

// HasValues returns a boolean if a field has been set.
func (o *InlineObject1255) HasValues() bool {
	if o != nil && o.Values != nil {
		return true
	}

	return false
}

// SetValues gets a reference to the given AnyOfobject and assigns it to the Values field.
func (o *InlineObject1255) SetValues(v AnyOfobject) {
	o.Values = v
}

func (o InlineObject1255) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.IndexNum != nil {
		toSerialize["indexNum"] = o.IndexNum
	}
	if o.Values != nil {
		toSerialize["values"] = o.Values
	}
	return json.Marshal(toSerialize)
}

type NullableInlineObject1255 struct {
	value *InlineObject1255
	isSet bool
}

func (v NullableInlineObject1255) Get() *InlineObject1255 {
	return v.value
}

func (v *NullableInlineObject1255) Set(val *InlineObject1255) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineObject1255) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineObject1255) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineObject1255(val *InlineObject1255) *NullableInlineObject1255 {
	return &NullableInlineObject1255{value: val, isSet: true}
}

func (v NullableInlineObject1255) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineObject1255) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


