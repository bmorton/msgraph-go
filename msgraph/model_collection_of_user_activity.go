/*
Partial Graph API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: v1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// CollectionOfUserActivity struct for CollectionOfUserActivity
type CollectionOfUserActivity struct {
	Value *[]MicrosoftGraphUserActivity `json:"value,omitempty"`
	OdataNextLink *string `json:"@odata.nextLink,omitempty"`
}

// NewCollectionOfUserActivity instantiates a new CollectionOfUserActivity object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCollectionOfUserActivity() *CollectionOfUserActivity {
	this := CollectionOfUserActivity{}
	return &this
}

// NewCollectionOfUserActivityWithDefaults instantiates a new CollectionOfUserActivity object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCollectionOfUserActivityWithDefaults() *CollectionOfUserActivity {
	this := CollectionOfUserActivity{}
	return &this
}

// GetValue returns the Value field value if set, zero value otherwise.
func (o *CollectionOfUserActivity) GetValue() []MicrosoftGraphUserActivity {
	if o == nil || o.Value == nil {
		var ret []MicrosoftGraphUserActivity
		return ret
	}
	return *o.Value
}

// GetValueOk returns a tuple with the Value field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CollectionOfUserActivity) GetValueOk() (*[]MicrosoftGraphUserActivity, bool) {
	if o == nil || o.Value == nil {
		return nil, false
	}
	return o.Value, true
}

// HasValue returns a boolean if a field has been set.
func (o *CollectionOfUserActivity) HasValue() bool {
	if o != nil && o.Value != nil {
		return true
	}

	return false
}

// SetValue gets a reference to the given []MicrosoftGraphUserActivity and assigns it to the Value field.
func (o *CollectionOfUserActivity) SetValue(v []MicrosoftGraphUserActivity) {
	o.Value = &v
}

// GetOdataNextLink returns the OdataNextLink field value if set, zero value otherwise.
func (o *CollectionOfUserActivity) GetOdataNextLink() string {
	if o == nil || o.OdataNextLink == nil {
		var ret string
		return ret
	}
	return *o.OdataNextLink
}

// GetOdataNextLinkOk returns a tuple with the OdataNextLink field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CollectionOfUserActivity) GetOdataNextLinkOk() (*string, bool) {
	if o == nil || o.OdataNextLink == nil {
		return nil, false
	}
	return o.OdataNextLink, true
}

// HasOdataNextLink returns a boolean if a field has been set.
func (o *CollectionOfUserActivity) HasOdataNextLink() bool {
	if o != nil && o.OdataNextLink != nil {
		return true
	}

	return false
}

// SetOdataNextLink gets a reference to the given string and assigns it to the OdataNextLink field.
func (o *CollectionOfUserActivity) SetOdataNextLink(v string) {
	o.OdataNextLink = &v
}

func (o CollectionOfUserActivity) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Value != nil {
		toSerialize["value"] = o.Value
	}
	if o.OdataNextLink != nil {
		toSerialize["@odata.nextLink"] = o.OdataNextLink
	}
	return json.Marshal(toSerialize)
}

type NullableCollectionOfUserActivity struct {
	value *CollectionOfUserActivity
	isSet bool
}

func (v NullableCollectionOfUserActivity) Get() *CollectionOfUserActivity {
	return v.value
}

func (v *NullableCollectionOfUserActivity) Set(val *CollectionOfUserActivity) {
	v.value = val
	v.isSet = true
}

func (v NullableCollectionOfUserActivity) IsSet() bool {
	return v.isSet
}

func (v *NullableCollectionOfUserActivity) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCollectionOfUserActivity(val *CollectionOfUserActivity) *NullableCollectionOfUserActivity {
	return &NullableCollectionOfUserActivity{value: val, isSet: true}
}

func (v NullableCollectionOfUserActivity) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCollectionOfUserActivity) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


