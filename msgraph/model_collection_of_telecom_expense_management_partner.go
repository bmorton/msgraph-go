/*
Partial Graph API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: v1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package msgraph

import (
	"encoding/json"
)

// CollectionOfTelecomExpenseManagementPartner struct for CollectionOfTelecomExpenseManagementPartner
type CollectionOfTelecomExpenseManagementPartner struct {
	Value *[]MicrosoftGraphTelecomExpenseManagementPartner `json:"value,omitempty"`
	OdataNextLink *string `json:"@odata.nextLink,omitempty"`
}

// NewCollectionOfTelecomExpenseManagementPartner instantiates a new CollectionOfTelecomExpenseManagementPartner object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCollectionOfTelecomExpenseManagementPartner() *CollectionOfTelecomExpenseManagementPartner {
	this := CollectionOfTelecomExpenseManagementPartner{}
	return &this
}

// NewCollectionOfTelecomExpenseManagementPartnerWithDefaults instantiates a new CollectionOfTelecomExpenseManagementPartner object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCollectionOfTelecomExpenseManagementPartnerWithDefaults() *CollectionOfTelecomExpenseManagementPartner {
	this := CollectionOfTelecomExpenseManagementPartner{}
	return &this
}

// GetValue returns the Value field value if set, zero value otherwise.
func (o *CollectionOfTelecomExpenseManagementPartner) GetValue() []MicrosoftGraphTelecomExpenseManagementPartner {
	if o == nil || o.Value == nil {
		var ret []MicrosoftGraphTelecomExpenseManagementPartner
		return ret
	}
	return *o.Value
}

// GetValueOk returns a tuple with the Value field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CollectionOfTelecomExpenseManagementPartner) GetValueOk() (*[]MicrosoftGraphTelecomExpenseManagementPartner, bool) {
	if o == nil || o.Value == nil {
		return nil, false
	}
	return o.Value, true
}

// HasValue returns a boolean if a field has been set.
func (o *CollectionOfTelecomExpenseManagementPartner) HasValue() bool {
	if o != nil && o.Value != nil {
		return true
	}

	return false
}

// SetValue gets a reference to the given []MicrosoftGraphTelecomExpenseManagementPartner and assigns it to the Value field.
func (o *CollectionOfTelecomExpenseManagementPartner) SetValue(v []MicrosoftGraphTelecomExpenseManagementPartner) {
	o.Value = &v
}

// GetOdataNextLink returns the OdataNextLink field value if set, zero value otherwise.
func (o *CollectionOfTelecomExpenseManagementPartner) GetOdataNextLink() string {
	if o == nil || o.OdataNextLink == nil {
		var ret string
		return ret
	}
	return *o.OdataNextLink
}

// GetOdataNextLinkOk returns a tuple with the OdataNextLink field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CollectionOfTelecomExpenseManagementPartner) GetOdataNextLinkOk() (*string, bool) {
	if o == nil || o.OdataNextLink == nil {
		return nil, false
	}
	return o.OdataNextLink, true
}

// HasOdataNextLink returns a boolean if a field has been set.
func (o *CollectionOfTelecomExpenseManagementPartner) HasOdataNextLink() bool {
	if o != nil && o.OdataNextLink != nil {
		return true
	}

	return false
}

// SetOdataNextLink gets a reference to the given string and assigns it to the OdataNextLink field.
func (o *CollectionOfTelecomExpenseManagementPartner) SetOdataNextLink(v string) {
	o.OdataNextLink = &v
}

func (o CollectionOfTelecomExpenseManagementPartner) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Value != nil {
		toSerialize["value"] = o.Value
	}
	if o.OdataNextLink != nil {
		toSerialize["@odata.nextLink"] = o.OdataNextLink
	}
	return json.Marshal(toSerialize)
}

type NullableCollectionOfTelecomExpenseManagementPartner struct {
	value *CollectionOfTelecomExpenseManagementPartner
	isSet bool
}

func (v NullableCollectionOfTelecomExpenseManagementPartner) Get() *CollectionOfTelecomExpenseManagementPartner {
	return v.value
}

func (v *NullableCollectionOfTelecomExpenseManagementPartner) Set(val *CollectionOfTelecomExpenseManagementPartner) {
	v.value = val
	v.isSet = true
}

func (v NullableCollectionOfTelecomExpenseManagementPartner) IsSet() bool {
	return v.isSet
}

func (v *NullableCollectionOfTelecomExpenseManagementPartner) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCollectionOfTelecomExpenseManagementPartner(val *CollectionOfTelecomExpenseManagementPartner) *NullableCollectionOfTelecomExpenseManagementPartner {
	return &NullableCollectionOfTelecomExpenseManagementPartner{value: val, isSet: true}
}

func (v NullableCollectionOfTelecomExpenseManagementPartner) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCollectionOfTelecomExpenseManagementPartner) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


