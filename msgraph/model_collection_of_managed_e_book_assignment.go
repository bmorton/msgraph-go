/*
Partial Graph API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: v1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package msgraph

import (
	"encoding/json"
)

// CollectionOfManagedEBookAssignment struct for CollectionOfManagedEBookAssignment
type CollectionOfManagedEBookAssignment struct {
	Value *[]MicrosoftGraphManagedEBookAssignment `json:"value,omitempty"`
	OdataNextLink *string `json:"@odata.nextLink,omitempty"`
}

// NewCollectionOfManagedEBookAssignment instantiates a new CollectionOfManagedEBookAssignment object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCollectionOfManagedEBookAssignment() *CollectionOfManagedEBookAssignment {
	this := CollectionOfManagedEBookAssignment{}
	return &this
}

// NewCollectionOfManagedEBookAssignmentWithDefaults instantiates a new CollectionOfManagedEBookAssignment object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCollectionOfManagedEBookAssignmentWithDefaults() *CollectionOfManagedEBookAssignment {
	this := CollectionOfManagedEBookAssignment{}
	return &this
}

// GetValue returns the Value field value if set, zero value otherwise.
func (o *CollectionOfManagedEBookAssignment) GetValue() []MicrosoftGraphManagedEBookAssignment {
	if o == nil || o.Value == nil {
		var ret []MicrosoftGraphManagedEBookAssignment
		return ret
	}
	return *o.Value
}

// GetValueOk returns a tuple with the Value field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CollectionOfManagedEBookAssignment) GetValueOk() (*[]MicrosoftGraphManagedEBookAssignment, bool) {
	if o == nil || o.Value == nil {
		return nil, false
	}
	return o.Value, true
}

// HasValue returns a boolean if a field has been set.
func (o *CollectionOfManagedEBookAssignment) HasValue() bool {
	if o != nil && o.Value != nil {
		return true
	}

	return false
}

// SetValue gets a reference to the given []MicrosoftGraphManagedEBookAssignment and assigns it to the Value field.
func (o *CollectionOfManagedEBookAssignment) SetValue(v []MicrosoftGraphManagedEBookAssignment) {
	o.Value = &v
}

// GetOdataNextLink returns the OdataNextLink field value if set, zero value otherwise.
func (o *CollectionOfManagedEBookAssignment) GetOdataNextLink() string {
	if o == nil || o.OdataNextLink == nil {
		var ret string
		return ret
	}
	return *o.OdataNextLink
}

// GetOdataNextLinkOk returns a tuple with the OdataNextLink field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CollectionOfManagedEBookAssignment) GetOdataNextLinkOk() (*string, bool) {
	if o == nil || o.OdataNextLink == nil {
		return nil, false
	}
	return o.OdataNextLink, true
}

// HasOdataNextLink returns a boolean if a field has been set.
func (o *CollectionOfManagedEBookAssignment) HasOdataNextLink() bool {
	if o != nil && o.OdataNextLink != nil {
		return true
	}

	return false
}

// SetOdataNextLink gets a reference to the given string and assigns it to the OdataNextLink field.
func (o *CollectionOfManagedEBookAssignment) SetOdataNextLink(v string) {
	o.OdataNextLink = &v
}

func (o CollectionOfManagedEBookAssignment) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Value != nil {
		toSerialize["value"] = o.Value
	}
	if o.OdataNextLink != nil {
		toSerialize["@odata.nextLink"] = o.OdataNextLink
	}
	return json.Marshal(toSerialize)
}

type NullableCollectionOfManagedEBookAssignment struct {
	value *CollectionOfManagedEBookAssignment
	isSet bool
}

func (v NullableCollectionOfManagedEBookAssignment) Get() *CollectionOfManagedEBookAssignment {
	return v.value
}

func (v *NullableCollectionOfManagedEBookAssignment) Set(val *CollectionOfManagedEBookAssignment) {
	v.value = val
	v.isSet = true
}

func (v NullableCollectionOfManagedEBookAssignment) IsSet() bool {
	return v.isSet
}

func (v *NullableCollectionOfManagedEBookAssignment) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCollectionOfManagedEBookAssignment(val *CollectionOfManagedEBookAssignment) *NullableCollectionOfManagedEBookAssignment {
	return &NullableCollectionOfManagedEBookAssignment{value: val, isSet: true}
}

func (v NullableCollectionOfManagedEBookAssignment) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCollectionOfManagedEBookAssignment) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


