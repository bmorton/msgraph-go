/*
Partial Graph API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: v1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package msgraph

import (
	"encoding/json"
)

// CollectionOfIosUpdateDeviceStatus struct for CollectionOfIosUpdateDeviceStatus
type CollectionOfIosUpdateDeviceStatus struct {
	Value *[]MicrosoftGraphIosUpdateDeviceStatus `json:"value,omitempty"`
	OdataNextLink *string `json:"@odata.nextLink,omitempty"`
}

// NewCollectionOfIosUpdateDeviceStatus instantiates a new CollectionOfIosUpdateDeviceStatus object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCollectionOfIosUpdateDeviceStatus() *CollectionOfIosUpdateDeviceStatus {
	this := CollectionOfIosUpdateDeviceStatus{}
	return &this
}

// NewCollectionOfIosUpdateDeviceStatusWithDefaults instantiates a new CollectionOfIosUpdateDeviceStatus object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCollectionOfIosUpdateDeviceStatusWithDefaults() *CollectionOfIosUpdateDeviceStatus {
	this := CollectionOfIosUpdateDeviceStatus{}
	return &this
}

// GetValue returns the Value field value if set, zero value otherwise.
func (o *CollectionOfIosUpdateDeviceStatus) GetValue() []MicrosoftGraphIosUpdateDeviceStatus {
	if o == nil || o.Value == nil {
		var ret []MicrosoftGraphIosUpdateDeviceStatus
		return ret
	}
	return *o.Value
}

// GetValueOk returns a tuple with the Value field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CollectionOfIosUpdateDeviceStatus) GetValueOk() (*[]MicrosoftGraphIosUpdateDeviceStatus, bool) {
	if o == nil || o.Value == nil {
		return nil, false
	}
	return o.Value, true
}

// HasValue returns a boolean if a field has been set.
func (o *CollectionOfIosUpdateDeviceStatus) HasValue() bool {
	if o != nil && o.Value != nil {
		return true
	}

	return false
}

// SetValue gets a reference to the given []MicrosoftGraphIosUpdateDeviceStatus and assigns it to the Value field.
func (o *CollectionOfIosUpdateDeviceStatus) SetValue(v []MicrosoftGraphIosUpdateDeviceStatus) {
	o.Value = &v
}

// GetOdataNextLink returns the OdataNextLink field value if set, zero value otherwise.
func (o *CollectionOfIosUpdateDeviceStatus) GetOdataNextLink() string {
	if o == nil || o.OdataNextLink == nil {
		var ret string
		return ret
	}
	return *o.OdataNextLink
}

// GetOdataNextLinkOk returns a tuple with the OdataNextLink field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CollectionOfIosUpdateDeviceStatus) GetOdataNextLinkOk() (*string, bool) {
	if o == nil || o.OdataNextLink == nil {
		return nil, false
	}
	return o.OdataNextLink, true
}

// HasOdataNextLink returns a boolean if a field has been set.
func (o *CollectionOfIosUpdateDeviceStatus) HasOdataNextLink() bool {
	if o != nil && o.OdataNextLink != nil {
		return true
	}

	return false
}

// SetOdataNextLink gets a reference to the given string and assigns it to the OdataNextLink field.
func (o *CollectionOfIosUpdateDeviceStatus) SetOdataNextLink(v string) {
	o.OdataNextLink = &v
}

func (o CollectionOfIosUpdateDeviceStatus) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Value != nil {
		toSerialize["value"] = o.Value
	}
	if o.OdataNextLink != nil {
		toSerialize["@odata.nextLink"] = o.OdataNextLink
	}
	return json.Marshal(toSerialize)
}

type NullableCollectionOfIosUpdateDeviceStatus struct {
	value *CollectionOfIosUpdateDeviceStatus
	isSet bool
}

func (v NullableCollectionOfIosUpdateDeviceStatus) Get() *CollectionOfIosUpdateDeviceStatus {
	return v.value
}

func (v *NullableCollectionOfIosUpdateDeviceStatus) Set(val *CollectionOfIosUpdateDeviceStatus) {
	v.value = val
	v.isSet = true
}

func (v NullableCollectionOfIosUpdateDeviceStatus) IsSet() bool {
	return v.isSet
}

func (v *NullableCollectionOfIosUpdateDeviceStatus) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCollectionOfIosUpdateDeviceStatus(val *CollectionOfIosUpdateDeviceStatus) *NullableCollectionOfIosUpdateDeviceStatus {
	return &NullableCollectionOfIosUpdateDeviceStatus{value: val, isSet: true}
}

func (v NullableCollectionOfIosUpdateDeviceStatus) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCollectionOfIosUpdateDeviceStatus) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


