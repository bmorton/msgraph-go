/*
Partial Graph API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: v1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// CollectionOfWorkbookChart struct for CollectionOfWorkbookChart
type CollectionOfWorkbookChart struct {
	Value *[]MicrosoftGraphWorkbookChart `json:"value,omitempty"`
	OdataNextLink *string `json:"@odata.nextLink,omitempty"`
}

// NewCollectionOfWorkbookChart instantiates a new CollectionOfWorkbookChart object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCollectionOfWorkbookChart() *CollectionOfWorkbookChart {
	this := CollectionOfWorkbookChart{}
	return &this
}

// NewCollectionOfWorkbookChartWithDefaults instantiates a new CollectionOfWorkbookChart object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCollectionOfWorkbookChartWithDefaults() *CollectionOfWorkbookChart {
	this := CollectionOfWorkbookChart{}
	return &this
}

// GetValue returns the Value field value if set, zero value otherwise.
func (o *CollectionOfWorkbookChart) GetValue() []MicrosoftGraphWorkbookChart {
	if o == nil || o.Value == nil {
		var ret []MicrosoftGraphWorkbookChart
		return ret
	}
	return *o.Value
}

// GetValueOk returns a tuple with the Value field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CollectionOfWorkbookChart) GetValueOk() (*[]MicrosoftGraphWorkbookChart, bool) {
	if o == nil || o.Value == nil {
		return nil, false
	}
	return o.Value, true
}

// HasValue returns a boolean if a field has been set.
func (o *CollectionOfWorkbookChart) HasValue() bool {
	if o != nil && o.Value != nil {
		return true
	}

	return false
}

// SetValue gets a reference to the given []MicrosoftGraphWorkbookChart and assigns it to the Value field.
func (o *CollectionOfWorkbookChart) SetValue(v []MicrosoftGraphWorkbookChart) {
	o.Value = &v
}

// GetOdataNextLink returns the OdataNextLink field value if set, zero value otherwise.
func (o *CollectionOfWorkbookChart) GetOdataNextLink() string {
	if o == nil || o.OdataNextLink == nil {
		var ret string
		return ret
	}
	return *o.OdataNextLink
}

// GetOdataNextLinkOk returns a tuple with the OdataNextLink field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CollectionOfWorkbookChart) GetOdataNextLinkOk() (*string, bool) {
	if o == nil || o.OdataNextLink == nil {
		return nil, false
	}
	return o.OdataNextLink, true
}

// HasOdataNextLink returns a boolean if a field has been set.
func (o *CollectionOfWorkbookChart) HasOdataNextLink() bool {
	if o != nil && o.OdataNextLink != nil {
		return true
	}

	return false
}

// SetOdataNextLink gets a reference to the given string and assigns it to the OdataNextLink field.
func (o *CollectionOfWorkbookChart) SetOdataNextLink(v string) {
	o.OdataNextLink = &v
}

func (o CollectionOfWorkbookChart) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Value != nil {
		toSerialize["value"] = o.Value
	}
	if o.OdataNextLink != nil {
		toSerialize["@odata.nextLink"] = o.OdataNextLink
	}
	return json.Marshal(toSerialize)
}

type NullableCollectionOfWorkbookChart struct {
	value *CollectionOfWorkbookChart
	isSet bool
}

func (v NullableCollectionOfWorkbookChart) Get() *CollectionOfWorkbookChart {
	return v.value
}

func (v *NullableCollectionOfWorkbookChart) Set(val *CollectionOfWorkbookChart) {
	v.value = val
	v.isSet = true
}

func (v NullableCollectionOfWorkbookChart) IsSet() bool {
	return v.isSet
}

func (v *NullableCollectionOfWorkbookChart) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCollectionOfWorkbookChart(val *CollectionOfWorkbookChart) *NullableCollectionOfWorkbookChart {
	return &NullableCollectionOfWorkbookChart{value: val, isSet: true}
}

func (v NullableCollectionOfWorkbookChart) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCollectionOfWorkbookChart) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


