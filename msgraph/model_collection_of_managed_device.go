/*
Partial Graph API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: v1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package msgraph

import (
	"encoding/json"
)

// CollectionOfManagedDevice struct for CollectionOfManagedDevice
type CollectionOfManagedDevice struct {
	Value *[]MicrosoftGraphManagedDevice `json:"value,omitempty"`
	OdataNextLink *string `json:"@odata.nextLink,omitempty"`
}

// NewCollectionOfManagedDevice instantiates a new CollectionOfManagedDevice object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCollectionOfManagedDevice() *CollectionOfManagedDevice {
	this := CollectionOfManagedDevice{}
	return &this
}

// NewCollectionOfManagedDeviceWithDefaults instantiates a new CollectionOfManagedDevice object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCollectionOfManagedDeviceWithDefaults() *CollectionOfManagedDevice {
	this := CollectionOfManagedDevice{}
	return &this
}

// GetValue returns the Value field value if set, zero value otherwise.
func (o *CollectionOfManagedDevice) GetValue() []MicrosoftGraphManagedDevice {
	if o == nil || o.Value == nil {
		var ret []MicrosoftGraphManagedDevice
		return ret
	}
	return *o.Value
}

// GetValueOk returns a tuple with the Value field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CollectionOfManagedDevice) GetValueOk() (*[]MicrosoftGraphManagedDevice, bool) {
	if o == nil || o.Value == nil {
		return nil, false
	}
	return o.Value, true
}

// HasValue returns a boolean if a field has been set.
func (o *CollectionOfManagedDevice) HasValue() bool {
	if o != nil && o.Value != nil {
		return true
	}

	return false
}

// SetValue gets a reference to the given []MicrosoftGraphManagedDevice and assigns it to the Value field.
func (o *CollectionOfManagedDevice) SetValue(v []MicrosoftGraphManagedDevice) {
	o.Value = &v
}

// GetOdataNextLink returns the OdataNextLink field value if set, zero value otherwise.
func (o *CollectionOfManagedDevice) GetOdataNextLink() string {
	if o == nil || o.OdataNextLink == nil {
		var ret string
		return ret
	}
	return *o.OdataNextLink
}

// GetOdataNextLinkOk returns a tuple with the OdataNextLink field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CollectionOfManagedDevice) GetOdataNextLinkOk() (*string, bool) {
	if o == nil || o.OdataNextLink == nil {
		return nil, false
	}
	return o.OdataNextLink, true
}

// HasOdataNextLink returns a boolean if a field has been set.
func (o *CollectionOfManagedDevice) HasOdataNextLink() bool {
	if o != nil && o.OdataNextLink != nil {
		return true
	}

	return false
}

// SetOdataNextLink gets a reference to the given string and assigns it to the OdataNextLink field.
func (o *CollectionOfManagedDevice) SetOdataNextLink(v string) {
	o.OdataNextLink = &v
}

func (o CollectionOfManagedDevice) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Value != nil {
		toSerialize["value"] = o.Value
	}
	if o.OdataNextLink != nil {
		toSerialize["@odata.nextLink"] = o.OdataNextLink
	}
	return json.Marshal(toSerialize)
}

type NullableCollectionOfManagedDevice struct {
	value *CollectionOfManagedDevice
	isSet bool
}

func (v NullableCollectionOfManagedDevice) Get() *CollectionOfManagedDevice {
	return v.value
}

func (v *NullableCollectionOfManagedDevice) Set(val *CollectionOfManagedDevice) {
	v.value = val
	v.isSet = true
}

func (v NullableCollectionOfManagedDevice) IsSet() bool {
	return v.isSet
}

func (v *NullableCollectionOfManagedDevice) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCollectionOfManagedDevice(val *CollectionOfManagedDevice) *NullableCollectionOfManagedDevice {
	return &NullableCollectionOfManagedDevice{value: val, isSet: true}
}

func (v NullableCollectionOfManagedDevice) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCollectionOfManagedDevice) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


