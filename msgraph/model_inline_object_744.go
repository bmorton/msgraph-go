/*
Partial Graph API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: v1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package msgraph

import (
	"encoding/json"
)

// InlineObject744 struct for InlineObject744
type InlineObject744 struct {
	ContentType *string `json:"contentType,omitempty"`
}

// NewInlineObject744 instantiates a new InlineObject744 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineObject744() *InlineObject744 {
	this := InlineObject744{}
	return &this
}

// NewInlineObject744WithDefaults instantiates a new InlineObject744 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineObject744WithDefaults() *InlineObject744 {
	this := InlineObject744{}
	return &this
}

// GetContentType returns the ContentType field value if set, zero value otherwise.
func (o *InlineObject744) GetContentType() string {
	if o == nil || o.ContentType == nil {
		var ret string
		return ret
	}
	return *o.ContentType
}

// GetContentTypeOk returns a tuple with the ContentType field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineObject744) GetContentTypeOk() (*string, bool) {
	if o == nil || o.ContentType == nil {
		return nil, false
	}
	return o.ContentType, true
}

// HasContentType returns a boolean if a field has been set.
func (o *InlineObject744) HasContentType() bool {
	if o != nil && o.ContentType != nil {
		return true
	}

	return false
}

// SetContentType gets a reference to the given string and assigns it to the ContentType field.
func (o *InlineObject744) SetContentType(v string) {
	o.ContentType = &v
}

func (o InlineObject744) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.ContentType != nil {
		toSerialize["contentType"] = o.ContentType
	}
	return json.Marshal(toSerialize)
}

type NullableInlineObject744 struct {
	value *InlineObject744
	isSet bool
}

func (v NullableInlineObject744) Get() *InlineObject744 {
	return v.value
}

func (v *NullableInlineObject744) Set(val *InlineObject744) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineObject744) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineObject744) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineObject744(val *InlineObject744) *NullableInlineObject744 {
	return &NullableInlineObject744{value: val, isSet: true}
}

func (v NullableInlineObject744) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineObject744) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


