/*
Partial Graph API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: v1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// CollectionOfDriveItem struct for CollectionOfDriveItem
type CollectionOfDriveItem struct {
	Value *[]MicrosoftGraphDriveItem `json:"value,omitempty"`
	OdataNextLink *string `json:"@odata.nextLink,omitempty"`
}

// NewCollectionOfDriveItem instantiates a new CollectionOfDriveItem object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCollectionOfDriveItem() *CollectionOfDriveItem {
	this := CollectionOfDriveItem{}
	return &this
}

// NewCollectionOfDriveItemWithDefaults instantiates a new CollectionOfDriveItem object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCollectionOfDriveItemWithDefaults() *CollectionOfDriveItem {
	this := CollectionOfDriveItem{}
	return &this
}

// GetValue returns the Value field value if set, zero value otherwise.
func (o *CollectionOfDriveItem) GetValue() []MicrosoftGraphDriveItem {
	if o == nil || o.Value == nil {
		var ret []MicrosoftGraphDriveItem
		return ret
	}
	return *o.Value
}

// GetValueOk returns a tuple with the Value field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CollectionOfDriveItem) GetValueOk() (*[]MicrosoftGraphDriveItem, bool) {
	if o == nil || o.Value == nil {
		return nil, false
	}
	return o.Value, true
}

// HasValue returns a boolean if a field has been set.
func (o *CollectionOfDriveItem) HasValue() bool {
	if o != nil && o.Value != nil {
		return true
	}

	return false
}

// SetValue gets a reference to the given []MicrosoftGraphDriveItem and assigns it to the Value field.
func (o *CollectionOfDriveItem) SetValue(v []MicrosoftGraphDriveItem) {
	o.Value = &v
}

// GetOdataNextLink returns the OdataNextLink field value if set, zero value otherwise.
func (o *CollectionOfDriveItem) GetOdataNextLink() string {
	if o == nil || o.OdataNextLink == nil {
		var ret string
		return ret
	}
	return *o.OdataNextLink
}

// GetOdataNextLinkOk returns a tuple with the OdataNextLink field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CollectionOfDriveItem) GetOdataNextLinkOk() (*string, bool) {
	if o == nil || o.OdataNextLink == nil {
		return nil, false
	}
	return o.OdataNextLink, true
}

// HasOdataNextLink returns a boolean if a field has been set.
func (o *CollectionOfDriveItem) HasOdataNextLink() bool {
	if o != nil && o.OdataNextLink != nil {
		return true
	}

	return false
}

// SetOdataNextLink gets a reference to the given string and assigns it to the OdataNextLink field.
func (o *CollectionOfDriveItem) SetOdataNextLink(v string) {
	o.OdataNextLink = &v
}

func (o CollectionOfDriveItem) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Value != nil {
		toSerialize["value"] = o.Value
	}
	if o.OdataNextLink != nil {
		toSerialize["@odata.nextLink"] = o.OdataNextLink
	}
	return json.Marshal(toSerialize)
}

type NullableCollectionOfDriveItem struct {
	value *CollectionOfDriveItem
	isSet bool
}

func (v NullableCollectionOfDriveItem) Get() *CollectionOfDriveItem {
	return v.value
}

func (v *NullableCollectionOfDriveItem) Set(val *CollectionOfDriveItem) {
	v.value = val
	v.isSet = true
}

func (v NullableCollectionOfDriveItem) IsSet() bool {
	return v.isSet
}

func (v *NullableCollectionOfDriveItem) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCollectionOfDriveItem(val *CollectionOfDriveItem) *NullableCollectionOfDriveItem {
	return &NullableCollectionOfDriveItem{value: val, isSet: true}
}

func (v NullableCollectionOfDriveItem) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCollectionOfDriveItem) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


